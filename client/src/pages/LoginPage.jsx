...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        addToast(`¡Bienvenido de nuevo, ${res.user.username}!`, 'success');
        navigate('/app');
      } else {
    ...
    <div style={{ maxWidth: '440px', margin: '60px auto', padding: '0 24px' }}>
      <div className="card-brutal animate-fade-in" style={{ padding: '32px' }}>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <h2 style={{ fontSize: '1.8rem', fontWeight: 900 }}>Iniciar Sesión</h2>
          <p style={{ color: 'var(--color-gray-text)', fontSize: '0.9rem', marginTop: '4px' }}>Accede a tu asistente inteligente Loop Kitchen</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            ...
            <input
              ...
              placeholder="ejemplo@loopkitchen.com"
              style={{
                width: '100%',
                padding: '12px 14px',
                borderRadius: '8px',
          ...
          <button type="submit" disabled={loading} className="btn-brutal" style={{ width: '100%', marginTop: '8px' }}>
            {loading ? 'Ingresando...' : <><LogIn size={18} /> Entrar a Loop Kitchen</>}
          </button>
        </form>

        <div style={{ textAlign: 'center', marginTop: '20px', fontSize: '0.85rem', color: 'var(--color-gray-text)' }}>
          ¿No tienes una cuenta? <Link to="/register" style={{ fontWeight: 700, color: 'var(--color-dark)', textDecoration: 'underline' }}>Regístrate aquí</Link>
        </div>
      </div>
    </div>
  );
